...
// MainnetGatewayContractSafeTransferFromFailedIterator is returned from FilterSafeTransferFromFailed and is used to iterate over the raw logs and unpacked data for SafeTransferFromFailed events raised by the MainnetGatewayContract contract.
type MainnetGatewayContractSafeTransferFromFailedIterator struct {
	Event *MainnetGatewayContractSafeTransferFromFailed // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *MainnetGatewayContractSafeTransferFromFailedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(MainnetGatewayContractSafeTransferFromFailed)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *MainnetGatewayContractSafeTransferFromFailedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *MainnetGatewayContractSafeTransferFromFailedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// MainnetGatewayContractSafeTransferFromFailed represents a SafeTransferFromFailed event raised by the MainnetGatewayContract contract.
type MainnetGatewayContractSafeTransferFromFailed struct {
	ContractAddress common.Address
	From            common.Address
	To              common.Address
	Uid             *big.Int
...
// FilterSafeTransferFromFailed is a free log retrieval operation binding the contract event 0x836dec169e2291985c6916658c9e90e855b101ab751227538cfe93ae6e1d0be1.
//
// Solidity: e SafeTransferFromFailed(ContractAddress address, from address, to address, uid uint256)
func (_MainnetGatewayContract *MainnetGatewayContractFilterer) FilterSafeTransferFromFailed(opts *bind.FilterOpts) (*MainnetGatewayContractSafeTransferFromFailedIterator, error) {

	logs, sub, err := _MainnetGatewayContract.contract.FilterLogs(opts, "SafeTransferFromFailed")
	if err != nil {
		return nil, err
	}
	return &MainnetGatewayContractSafeTransferFromFailedIterator{contract: _MainnetGatewayContract.contract, event: "SafeTransferFromFailed", logs: logs, sub: sub}, nil
}

// WatchSafeTransferFromFailed is a free log subscription operation binding the contract event 0x836dec169e2291985c6916658c9e90e855b101ab751227538cfe93ae6e1d0be1.
//
// Solidity: e SafeTransferFromFailed(ContractAddress address, from address, to address, uid uint256)
func (_MainnetGatewayContract *MainnetGatewayContractFilterer) WatchSafeTransferFromFailed(opts *bind.WatchOpts, sink chan<- *MainnetGatewayContractSafeTransferFromFailed) (event.Subscription, error) {

	logs, sub, err := _MainnetGatewayContract.contract.WatchLogs(opts, "SafeTransferFromFailed")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(MainnetGatewayContractSafeTransferFromFailed)
				if err := _MainnetGatewayContract.contract.UnpackLog(event, "SafeTransferFromFailed", log); err != nil {
					return err
				}
				event.Raw = log
