// Example command
// `truffle exec scripts/withdraw_eth_v2.js --network rinkeby --dapp-network asia1`
//
...
module.exports = async function (callback) {
  ...
  try {
    ...
    // sig, amount, and token address can be retreived via `$LOOM_BIN gateway withdrawal-receipt eth:account` command
    // base64 signature
    const sig = ''
    // amount in big int
    const amount = new BN('1000000000000000000', 10)
    // token address in hex format 
    const tokenAddress = gateway.address

    if (!sig || !amount || !tokenAddress) {
      throw new Error("sig, amount, and token tokenAddress are required")
    }

    const prefix = MessagePrefix.ETHPrefix
    const params = {
      prefix,
      gatewayAddress,
      amount,
    ...
    let tx = await gateway.withdrawETH(
      amount,
      valIndexes,
      vs,
      rs,
